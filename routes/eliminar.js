...
router.post('/material', isLoggedIn, async(req, res) => {
    const { sku } = req.body;
    if (await pool.query('DELETE FROM material where sku = ?', [sku])) {
...
router.post('/usuario', isLoggedIn, async(req, res) => {
    const { mb } = req.body;
    if (await pool.query('DELETE FROM registro where mb = ?', [mb])) {
...
router.post('/actividad', isLoggedIn, async(req, res) => {
    const { cod } = req.body;
    if (await pool.query('DELETE FROM actividad WHERE cod = ?', [cod])) {
...
router.post('/eliminar_deA', isLoggedIn, async(req, res) => {
    const { u, c } = req.body;
    if (await pool.query('DELETE FROM inscritos WHERE id_mb = ? AND id_act = ?', [u, c])) {
